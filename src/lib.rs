...
pub fn apply_patch(patch: &Patch, src: &[u8]) -> UpsApplyResult<Vec<u8>> {
...
pub fn revert_patch(patch: &Patch, dst: &[u8]) -> UpsApplyResult<Vec<u8>> {
    if dst.len() != patch.dst_size {
    ...
    let mut output = dst[..patch.src_size].to_vec();
    let mut output_ptr: &mut [u8] = &mut output;
    for hunk in &patch.hunks {
